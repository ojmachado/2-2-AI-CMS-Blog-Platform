...
export const AdminDebug: React.FC = () => {
  ...
  const checkDb = async () => {
    ...
    try {
      ...
      // Catch fetch errors (e.g., API unreachable)
      setStatus({ connected: false, error: "API Unreachable. The backend might be offline or misconfigured." });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-20">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
            ...
            <p className="text-slate-500 mt-1 text-lg">Análise da conectividade do Banco de Dados (Neon/Vercel).</p>
        </div>
                    ...
                    <div className="mb-6 p-5 bg-red-50 text-red-800 rounded-2xl border border-red-100 font-mono text-xs overflow-auto">
                        ...
                        {status.error.includes('API Unreachable') && (
                            <div className="mt-4 pt-3 border-t border-red-200 text-red-900">
                                ...
                                <p className="font-sans">Este erro indica que a API do backend (`/api/nile`) falhou criticamente (crash) e não retornou uma resposta JSON. Verifique os logs da função correspondente no painel da Vercel para encontrar o erro exato que causou a falha. (Nota: a rota `/api/nile` é nomeada por razões históricas e se conecta ao seu banco de dados Neon).</p>
                            </div>
                        )}
                    </div>
                )}
