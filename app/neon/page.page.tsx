...
export default async function NeonTestPage() {
  ...
  try {
    ...
    // 2. CORREÇÃO DE SCHEMA: Tenta adicionar a coluna created_at se ela não existir
    // Isso resolve o erro "column created_at does not exist" se a tabela for antiga
    try {
        await sql('ALTER TABLE comments ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT NOW()');
    } catch (ignore) {
        // Ignora erros de alteração se a coluna já existir ou se o banco não suportar IF NOT EXISTS (Postgres antigo)
    }
  ...
  async function create(formData: FormData) {
    ...
    if (comment) {
        await sql('INSERT INTO comments (comment, created_at) VALUES ($1, NOW())', [comment.toString()]);
        revalidatePath('/neon');
    }
  }
                        ...
                        comments.map((item: any) => (
                            <div key={item.id} className="bg-white border border-slate-100 p-4 rounded-2xl shadow-sm flex flex-col gap-1 hover:border-indigo-100 transition-colors">
                                ...
                                <span className="text-[10px] text-slate-400 font-mono">
                                    {item.created_at ? new Date(item.created_at).toLocaleString() : 'Data Indisponível'}
                                </span>
                            </div>
                        ))
                    )}
